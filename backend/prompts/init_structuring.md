...
5. **`completeness_status` immer `teilweise`** setzen — nie `vollstaendig`. Dies ist eine Initialisierung, kein fertiges Artefakt. Der Strukturierungsdialog vertieft die Schritte und setzt `vollstaendig` wenn alles spezifikationsreif ist.
...
**s1** — Neue Bestellung im Webshop auswählen [aktion, reihenfolge 1, []→ → s2, completeness_status: teilweise]
...
**s2** — Kunde in SAP vorhanden? [entscheidung, reihenfolge 2, [s1]→, bedingung: "Existiert die Kundennummer aus der Webshop-Bestellung bereits als Debitor in SAP?", Ja → s3, Nein → s2a, konvergenz: s3, completeness_status: teilweise]
...
**s3** — Alle Bestellpositionen erfassen [schleife, reihenfolge 4, [s2,s2a]→, schleifenkoerper: [s3a], abbruchbedingung: "Alle Artikelpositionen der Webshop-Bestellung wurden in den SAP-Auftrag übertragen", → s4, completeness_status: teilweise]
...
**s4** — Versandart bestimmen [entscheidung, reihenfolge 6, [s3]→, regeln: (1) "Bestellwert > 500 € und Kunde hat Expressversand gewählt" → s4a (Express), (2) "Mindestens ein Artikel ist in SAP als sperrig gekennzeichnet" → s4b (Spedition), (3) Sonst → s4c (Standardversand), nachfolger: [s4a, s4b, s4c], konvergenz: s5, completeness_status: teilweise]
...
**s4a** — Expressversand beauftragen [aktion, reihenfolge 7, [s4]→ → s5, completeness_status: teilweise]
...
**s4c** — Standardversand auswählen [aktion, reihenfolge 9, [s4]→ → s5, completeness_status: teilweise]
...
**s5** — Auftrag sichern und Webshop aktualisieren [aktion, reihenfolge 10, [s4a,s4b,s4c]→ → s6, completeness_status: teilweise]
...
**s6** — Auftragsbestätigung per E-Mail versenden [aktion, reihenfolge 11, [s5]→ → [], completeness_status: teilweise]
...
**s_err_sap** — SAP nicht erreichbar [ausnahme, reihenfolge 99, []→ → [], completeness_status: teilweise]
