...
- **Entscheidungen richtig modellieren**: DECISION erlaubt **beliebig viele Regeln** (nicht nur 2). Regeln werden von oben nach unten ausgewertet — die erste wahre Regel gewinnt. Letzte Regel immer als Catch-All anlegen (`1 = 1`), damit der Prozess nie abbricht.
- **Schleifen mit Abbruchbedingung**: LOOP definiert eine maximale Iterationszahl. Innerhalb der Schleife kann eine DECISION als vorzeitige Abbruchbedingung dienen. Beachte: Vorzeitiger Abbruch markiert die Schleife als "nicht erfolgreich" — das kann für nachfolgende Entscheidungen relevant sein.
