"""Manueller CLI-Test für einen vollständigen Orchestrator-Turn.

Verwendung (aus backend/):
    python -m scripts.test_turn
    python -m scripts.test_turn "Ich möchte den Bestellprozess digitalisieren"
    python -m scripts.test_turn --turns 3

Voraussetzungen:
    - backend/.env mit gültigem LLM_API_KEY
    - venv aktiviert
"""
...
def _print_artifact(project: Project) -> None:
    ...
    print(f"  ExplorationArtifact — {len(filled)}/{len(slots)} Slots befüllt")
    for slot_id, slot in slots.items():
        status = "✓" if slot.inhalt and slot.inhalt.strip() else "○"
...
async def run(messages: list[str]) -> None:
    ...
    # In-Memory-DB für den Test (kein Dateisystem-Artefakt)
    db = Database(":memory:")
...
def main() -> None:
    ...
    parser.add_argument(
        "message",
        nargs="?",
        default="Ich möchte unseren Bestellprozess digitalisieren. Wir verarbeiten täglich etwa 200 Bestellungen manuell per E-Mail.",
        help="Nutzernachricht für Turn 1",
    )
    parser.add_argument(
        "--turns",
        type=int,
    ...
    follow_ups = [
        "Die Bestellungen kommen per E-Mail und manchmal per Telefon. Zuständig sind drei Mitarbeiterinnen im Innendienst.",
        "Das größte Problem ist die manuelle Dateneingabe ins ERP-System. Das dauert manchmal bis zu 30 Minuten pro Bestellung.",
        "Als Ergebnis soll eine Bestellung automatisch ins ERP übertragen werden, sobald sie per E-Mail eingeht.",
    ]
